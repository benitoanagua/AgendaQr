...
- **Adaptación deliberada**: se omitió la lente Hover (la referencia es web/wasm; AgendaQr es táctil Android/iOS y no hay superficie de puntero). No se copiaron nombres `Ww*`, dominio ni mecánicas.

## Aislamiento de producción

- El lab vive en `core:ui` pero **no está referenciado por ninguna pantalla de producción**; la app de producto (`AgendaQrApp`) no lo importa.
- El único host es la actividad debug-only `androidApp/src/debug/kotlin/com/agendaqr/android/lab/ComponentLabActivity.kt`, con manifiesto en `androidApp/src/debug/AndroidManifest.xml` y dependencia `debugImplementation(project(":core:ui"))`. Los builds de release no contienen la actividad ni el host del lab.
- El lab no accede a navegación, repositorios, servicios, autenticación, cámara, almacenamiento ni notificaciones.

## Cómo abrirlo (Android)

```bash
./gradlew :androidApp:assembleDebug
adb install -r androidApp/build/outputs/apk/debug/androidApp-debug.apk   # o :androidApp:installDebug
adb shell am start -n com.agendaqr.app/com.agendaqr.android.lab.ComponentLabActivity
```

La actividad no es exportada y no aparece en el launcher: solo se abre por ADB o desde Android Studio (Edit Configurations → Launch Options → Specified Activity).
...
- Compose Multiplatform según las convenciones del repositorio; el lab compila en los targets configurados de `core:ui` (Android + iOS).
...
## Validación ejecutada (host Linux x86_64, JDK 17)

| Comando | Resultado |
|---|---|
| `./gradlew :core:ui:build` | **PASS** — compila Android debug/release + iOS (iosX64/iosArm64/iosSimulatorArm64), lint y tests. |
| `:core:ui:testDebugUnitTest` / `testReleaseUnitTest` | **PASS** — 26 tests (11 integridad + 8 búsqueda + 7 matriz de interacción), 0 fallos en ambas variantes. |
| `compileTestKotlinIos*` | **PASS** — el commonTest compila como klibrary para los tres targets iOS. |
| `./gradlew verifyAgendaQrArchitecture` | **PASS** — compuertas de design system y de boundaries (fallaban en main antes de este cambio). |
| `./gradlew :androidApp:assembleDebug` | **PASS** — incluye el host debug-only del lab. |
| `./gradlew :androidApp:assembleRelease` | **PASS** — sin la actividad del lab (verificado en el manifiesto mergeado del APK). |

Pruebas añadidas (`core/ui/src/commonTest/kotlin/com/agendaqr/core/ui/lab/`):

- `LabCatalogIntegrityTest` (11): ids únicos, metadatos obligatorios, estados clasificados y descritos, tokens referenciados existentes, cobertura exacta de los componentes y fundamentos reales de `core:ui`, honestidad del soporte de tema oscuro, categorías válidas.
...
1. **Revisión visual manual pendiente**: se validó compilación y pruebas, no el aspecto visual. No se declara validación visual sin ejecutar la app.
2. **Ejecución de tests iOS**: los tests compilan para los tres targets iOS (klibrary) pero no pueden ejecutarse en este host Linux (requieren macOS + simulador). La compilación de los targets iOS sí está validada por `:core:ui:build`.
...
1. Revisión visual manual del lab en un dispositivo/emulador Android (claro y oscuro, vertical/horizontal, ancho estrecho y tablet).
